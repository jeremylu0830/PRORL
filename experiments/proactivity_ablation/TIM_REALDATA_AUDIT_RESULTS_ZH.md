...
這個固定 TIM evaluation week **目前沒有證明需要 RL**，而且 scalarized utility 有時會獎勵明顯更差的 SLA。後續 static controls、完整 K=50..201 與 validation-only tie selection 顯示：在約 20% violation rate 附近，dynamic exhaustive 可能以 movement 換取較低 surplus；但 scalar utility 與嚴格 SLA 仍可由 static 做得更好。這提供 dynamic relocation 可能有價值的證據，不構成跨週必要性或 RL 必要性證據。完整結果見 `TIM_STATIC_PARETO_VALIDATION_RESULTS_ZH.md` 及其引用的前序文件。
...
- 在 `(0.75, 0.2, 0.05)`，exhaustive search 相對 uniform wait 同時改善 utility 與 SLA；但 train-only utility-best-static 以零搬移取得更高 utility，train-only SLA-best-static 又以零搬移把 violations 降至 4。因此它只證明 uniform allocation 不佳，不構成動態調度必要性的證據。
...
因此不能從單一 evaluation week 宣稱「所有真實 TIM traffic 都不需要 RL」，但也不能用目前 benchmark 支持複雜 RL。較精確的結論是：**兩個設定直接偏好 uniform static；0.75 的 utility 與 SLA 都可分別由更強的 train-only static allocation 超越；0.35 則暴露 reward misalignment。**
...
3. **先定義 movement-necessary slice**：0.75/0.20/0.05 的 SLA-best-static 只剩 4/168 violations，現有 slice 不足以支持動態方法；應改用多個 evaluation windows 或更緊資源設定尋找 static 無法滿足的 workload。
...
後續 SLA-best-static control 已完成；詳細數值與完整性檢查見 `TIM_SLA_BEST_STATIC_RESULTS_ZH.md`。
